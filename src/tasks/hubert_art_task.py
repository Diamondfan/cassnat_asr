...
from models import make_hubert_art_model
...
class HubertArtTask(BaseTask):
    def __init__(self, mode, args):
        ...
        super(HubertArtTask, self).__init__(args)
    ...
    def set_model(self, args):
        self.model = make_hubert_art_model(args)
            ...
            try:
                if scaler is not None:
                    with torch.cuda.amp.autocast(dtype=torch.float16):
                        ctc_out, att_out, loss, att_loss, ctc_loss, src_size = self.model(src, tgt, src_mask, tgt_mask, audio_sizes, label_sizes, tgt_label, args.mask_prob)
                else:
                    ctc_out, att_out, loss, att_loss, ctc_loss, src_size = self.model(src, tgt, src_mask, tgt_mask, audio_sizes, label_sizes, tgt_label, args.mask_prob)
    ...
    def decode(self, args):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.test_loader):
                utt_list, audios, _, audio_sizes, _ = data
                src, src_mask = audios, (audios == args.padding_idx) #all audio except files which are masked
                                                                                    #check first col to see if file is masked
                #src_mask: ratios of unmasked length
                src_mask = src_mask.sum(-1) / src.size(-1)
                ...
                if args.decode_type == 'ctc_only':
                    ...
                    recog_results = self.model.fast_decode_with_ctc(src, src_mask, self.tokenizer.vocab, args, self.lm_model)
