...
def main_worker(rank, world_size, args, backend='nccl'):
    ...
    criterion_att2 = LabelSmoothing(args.vocab_size, args.padding_idx, 0)
    criterion = (criterion_ctc, criterion_att, criterion_att2)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.distributed:
            average_number = torch.Tensor([train_loss, train_wer, train_ctc_wer, train_wer2, valid_loss, valid_wer, valid_ctc_wer, valid_wer2]).float().cuda(args.rank)
            torch.distributed.all_reduce(average_number, op=ReduceOp.SUM)
            train_loss, train_wer, train_ctc_wer, train_wer2, valid_loss, valid_wer, valid_ctc_wer, valid_wer2 = (average_number / args.world_size).cpu().numpy()

        if args.rank == 0:
            print("Epoch {} done, Train Loss: {:.4f}, Train WER1: {:.4f} WER2: {:.4f} ctc WER: {:.4f} Valid Loss: {:.4f} Valid WER1: {:.4f} WER2: {:.4f} ctc WER: {:.4f} Current LR: {:4e}".format(
                        epoch, train_loss, train_wer, train_wer2, train_ctc_wer, valid_loss, valid_wer, valid_wer2, valid_ctc_wer, temp_lr), flush=True)
...
def run_epoch(epoch, dataloader, model, criterion, args, optimizer=None, is_train=True):
    batch_time = util.AverageMeter('Time', ':6.2f')
    losses = util.AverageMeter('Loss', ':.2e')
    ctc_losses = util.AverageMeter('CtcLoss', ":.2e")
    att_losses = util.AverageMeter('AttLoss', ":.2e")
    embed_losses = util.AverageMeter('Att2Loss', ":.2e")
    ctc_wers = util.AverageMeter('CtcWer', ':.4f')
    att_wers = util.AverageMeter('AttWer', ':.4f')
    att2_wers = util.AverageMeter('Att2Wer', ':.4f')
    token_speed = util.AverageMeter('TkSpd', ":.2f")
    progress = util.ProgressMeter(len(dataloader), batch_time, losses, embed_losses, ctc_losses, att_losses, ctc_wers, \
                                    att_wers, att2_wers, token_speed, prefix="Epoch:[{}]".format(epoch))
    ...
    for i, data in enumerate(dataloader):
        ...
        ctc_out, att_out, att_out2 = model(src, src_mask, feat_sizes, tgt_label, label_sizes, args, tgt=tgt)
        ...
        if args.embed_alpha > 0:
            embed_loss = criterion[2](att_out2.view(-1, att_out2.size(-1)), tgt_label.view(-1))
        ...
        att2_wers.update(att2_errs/all_tokens, all_tokens)
        ...
        if i % args.print_freq == 0 and args.rank == 0:
            progress.print(i)
    return losses.avg, att_wers.avg, ctc_wers.avg, att2_wers.avg
