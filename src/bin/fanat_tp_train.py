...
from utils.loss import LabelSmoothing, KLDivLoss, TPLoss
...
def main():
    ...
    parser.add_argument("--tp_alpha", default=0, type=float, help="Task ratio of trigger predictor")
...
def main_worker(rank, world_size, args, backend='nccl'):
    ...
    criterion_tp = TPLoss()
    criterion = (criterion_ctc, criterion_att, criterion_tp)    
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_wer, train_ctc_wer, train_tp_acc = run_epoch(epoch, train_loader, model, criterion, args, optimizer, is_train=True)
        
        model.eval()
        with torch.no_grad():
            args.sample_dist = 0
            valid_loss, valid_wer, valid_ctc_wer, valid_tp_acc = run_epoch(epoch, valid_loader, model, criterion, args, is_train=False)

        temp_lr = optimizer.param_groups[0]['lr'] if args.opt_type == "normal" else optimizer.optimizer.param_groups[0]['lr']
        if args.distributed:
            average_number = torch.Tensor([train_loss, train_wer, train_ctc_wer, train_tp_acc, valid_loss, valid_wer, valid_ctc_wer, valid_tp_acc]).float().cuda(args.rank)
            torch.distributed.all_reduce(average_number, op=ReduceOp.SUM)
            train_loss, train_wer, train_ctc_wer, train_tp_acc, valid_loss, valid_wer, valid_ctc_wer, valid_tp_acc = (average_number / args.world_size).cpu().numpy()

        if args.rank == 0:
            print("Epoch {} done, Train Loss: {:.4f}, Train WER: {:.4f} Train ctcWER: {:.4f} Train TpAcc: {:.4f} Valid Loss: {:.4f} Valid WER: {:.4f} Valid ctcWER: {:.4f} Valid TpAcc: {:.4f} Current LR: {:4e}".format(
                        epoch, train_loss, train_wer, train_ctc_wer, train_tp_acc, valid_loss, valid_wer, valid_ctc_wer, valid_tp_acc, temp_lr), flush=True)
...
def run_epoch(epoch, dataloader, model, criterion, args, optimizer=None, is_train=True):
    ...
    tp_losses = util.AverageMeter('TpLoss', ":.4e")
    ctc_wers = util.AverageMeter('CtcWer', ':.4f')
    att_wers = util.AverageMeter('AttWer', ':.4f')
    tp_accs = util.AverageMeter('TpAcc', ':.4f')
    token_speed = util.AverageMeter('TokenSpeed', ":.2f")
    progress = util.ProgressMeter(len(dataloader), batch_time, tp_losses, ctc_losses, att_losses, ctc_wers, \
                                    att_wers, tp_accs, token_speed, prefix="Epoch: [{}]".format(epoch))
    ...
    for i, data in enumerate(dataloader):
        ...
        ctc_out, tp_out, att_out, tgt_mask_pred, aligned_seq, src_mask_new = model(src, src_mask, feat_sizes, tgt_label, label_sizes, args, tgt=tgt)
        ...
        if args.tp_alpha > 0:
            tp_loss, tp_acc, tp_all = criterion[2](tp_out, aligned_seq, src_mask_new)
            loss += args.tp_alpha * tp_loss
        else:
            tp_loss = torch.Tensor([0])
        ...
        tp_accs.update(tp_acc/tp_all, tp_all)
        ...
        tp_losses.update(tp_loss.item(), tokens)
        ...
        if i % args.print_freq == 0 and args.rank == 0:
            progress.print(i)
    return losses.avg, att_wers.avg, ctc_wers.avg, tp_accs.avg
