...
class AcousticExtracter(nn.Module):
    "Extractor character level acoustic embedding with CTC segments"
    def __init__(self, size, src_attn, feed_forward, dropout):
        super(AcousticExtracter, self).__init__()
        self.src_attn = src_attn
        self.feed_forward = feed_forward
        self.sublayer = clones(SublayerConnection(size, dropout), 2)
        self.size = size
    ...
    def forward(self, x, memory, trigger_mask):
        y = self.sublayer[0](x, lambda x: self.src_attn(x, memory, memory, trigger_mask))
