...
class CASSNAT2(nn.Module):
    ...
    def forward(self, src, src_mask, src_size, tgt_label, label_sizes, tokenizer, text_encoder_tokenizer, num_updates, args):
        ...
        # loss computation
        att_loss = self.att_loss(att_out.reshape(-1, att_out.size(-1)), tgt_label.reshape(-1))

        loss += args.att_alpha * att_loss
        
        if args.interce_alpha > 0:
            interce_loss = self.interce_loss(interce_out.reshape(-1, interce_out.size(-1)), tgt_label.reshape(-1))
            loss += args.interce_alpha * interce_loss

        return ctc_out, att_out, loss, ctc_loss, att_loss, interce_out
    ...
    def beam_decode(self, src, x_mask, src_size, args, lm_model, tokenizer, text_encoder_tokenizer, ctc_top_seqs=None, labels=None, label_sizes=None):
        ...
        pe = self.pe.type_as(src).unsqueeze(0).repeat(bs, 1, 1)[:,:ymax,:]
        token_acoustic_embed = self.taee(pe, enc_h, trigger_mask)
        pred_embed = self.sad(token_acoustic_embed, tgt_mask1, args.selfce_alpha, args.interce_layer)
        interce_out = self.interce_generator(pred_embed[-1])
        ...
        # 5. obtain text_embed from interce out
        text_inputs = []
        inter_results = interce_out.argmax(-1)
        for cand in range(inter_results.size(0)):
            text = tokenizer.tokens2text(inter_results[cand].cpu().numpy())
