...
from models.gpt2.gpt2_config import GPT2Config

def make_model(text_encoder_path, model_name):
    # load config
    config_file = os.path.join(text_encoder_path, model_name, 'hparams.json')
    gpt2_config = GPT2Config()
    gpt2_config.load_from_json(config_file)
...
class LayerNorm(nn.Module):
    def __init__(self, features, eps=1e-12):
        """Normalize to mean=0, std=1, then do a diagonal affine transform, biased one"""
    ...
    def forward(self, x):
        ...
        x = (x - m) / torch.sqrt(s + self.epsilon)
        return self.weight * x + self.bias

class Conv1D(nn.Module):
    def __init__(self, nf, nx, init_stdev=0.02):
        super(Conv1D, self).__init__()
        self.nf = nf
        self.weight = Parameter(torch.empty(nx, nf))
        nn.init.normal_(self.weight, std=init_stdev)
...
class GPT2LMHeadModel(nn.Module):
    ...
    def extract_features(self, input_ids, position_ids=None, token_type_ids=None, past=None):
        hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past)
        return hidden_states, presents
