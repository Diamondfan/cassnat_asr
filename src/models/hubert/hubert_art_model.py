...
def make_model(args):
    ...
    model = HubertArt(
        hub_base, projection_layer, interctc_projection_layer,
...
class HubertArt(nn.Module):
    def __init__(self, hub_base, projection_layer, interctc_projection_layer, tgt_embed, decoder, ctc_gen, att_gen, interctc_gen, args):
        super(HubertArt, self).__init__()
    ...
    def forward(self, src, tgt, src_mask, tgt_mask, feat_sizes, label_sizes, tgt_label, mask_prob):
        enc_h, x_mask, layer_results = self.hub_base(src, padding_mask=src_mask, mask_prob=mask_prob)
    ...
    def forward_att(self, src, tgt, src_mask, tgt_mask):
        enc_h, x_mask, layer_results = self.hub_base(src, padding_mask=src_mask)
        enc_h = self.projection_layer(enc_h)
        x_mask = x_mask.unsqueeze(1)
        x_mask = ~x_mask
        dec_h = self.decoder(self.tgt_embed(tgt), enc_h, x_mask, tgt_mask)
    ...
    def beam_decode(self, src, src_mask, tokenizer, args, lm_model=None):
        ...
        enc_h, x_mask, _ = self.hub_base(src, padding_mask=src_mask, mask_prob=args.mask_prob)
        enc_h = self.projection_layer(enc_h)
        x_mask = x_mask.unsqueeze(1)
        x_mask = ~x_mask
        src_mask = x_mask
