...
def make_model(args):
    ...
    model = HubertNAT2(hub_base, projection_layer, interctc_projection_layer, 
                        Extra, Sad, Mad, c(generator), c(generator), pe, 
                        text_encoder, dim_map, interctc_gen, interce_gen, args)
...
class HubertNAT2(nn.Module):
    def __init__(self, hub_base, projection_layer, interctc_projection_layer, acembed_extractor, embed_mapper, decoder, ctc_gen, att_gen, pe, 
                        text_encoder, dim_map, interctc_gen, interce_gen, args):
        super(HubertNAT2, self).__init__()
    ...
    def forward(self, src, src_mask, src_size, tgt_label, label_sizes, tokenizer, text_encoder_tokenizer, num_updates, args):
        ...
        # 5. obtain text_embed from interce_output
        if num_updates < 6000:
            batch_size, seq_len, _ = interce_out.size()
            d_model = pred_embed[0].size(-1)
            text_embed = torch.zeros(batch_size, seq_len, d_model).type_as(interce_out)
            text_mask = (text_embed[:,:,0] != 0).unsqueeze(1)
        else:
            text_inputs = []
            inter_results = interce_out.argmax(-1)
            for cand in range(inter_results.size(0)):
                text = tokenizer.tokens2text(inter_results[cand].cpu().numpy())  
                #if num_updates % 20000 == 0:
                #    self.threshold = min(self.threshold + 0.05, 0.95)
                # should mask the word as well

                if args.with_mask:
                    probs = interce_out[cand].max(-1)[0].exp().cpu().detach().numpy()
                    word_order = []
                    n = -1
                    flag = 0
                    ...
                    mask_word = mask * word_order
                    mask_word = mask_word[mask_word!=0]

                    for i in range(len(mask_word)):
                        word_order[word_order == mask_word[i]] = len(word_order)
                    
                    word_order[word_order != len(word_order)] = False
                    word_order[word_order == len(word_order)] = True 
                    text = np.array(text)

                    if len(text[word_order==True]) > 0:
                        text[word_order==True] = "▁mask"
                    text = text.tolist()
            ...
            text_input = torch.zeros(bs, token_max).type_as(inter_results)
            for b in range(bs):
                text_input[b,:len(text_inputs[b])] = torch.Tensor(text_inputs[b]).type_as(inter_results)
        ...
        dec_h = self.decoder(pred_embed, enc_h,text_embed, x_mask, text_mask, tgt_mask, args.mixce_alpha, args.interce_layer)
        ...
        return ctc_out, att_out, loss, ctc_loss, att_loss, interce_out, src_size
