        ...
        for b in range(bs):
            ...
            for seq in batch_top_seqs[b]:
                ...
                for c in top_indices[b, t]:
                    ...
                    p_total = np.logaddexp(new_p_b, new_p_nb)
                    if lm_model is not None:
                        score_lm += lm_prob[s_idx, c].item() * args.lm_weight
            ...
            sort_f = lambda x: x['score_ctc'] + x['score_lm'] + args.length_penalty * len(x['hyp']) \
                        if args.length_penalty is not None else lambda x: x['score_ctc'] + x['score_lm']
            batch_top_seqs[b] = sorted(new_beam, key=sort_f, reverse=True)[:beam_width]
    return batch_top_seqs

def beam_search_path(model, src, src_mask, src_size, vocab, args, lm_model=None):
    ...
    batch_top_seqs = [ [{'ys': ys, 'score_ctc': 0.0, 'score_lm': 0.0, 'hyp': []}] for b in range(bs) ]
